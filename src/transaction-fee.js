...
export function prepareWithDynamicFee({ network, priorityFeerate, fundingSompi, reservedSompi = 0n, changeScriptPublicKey, build }) {
  ...
  for (let round = 0; round < MAX_REPRICING_ROUNDS; round += 1) {
    ...
    const { mass, assumedSignedInputs } = estimateSignedTransactionMass(network, prepared.transaction);
    const repricedFee = BigInt(Math.ceil(mass * rate));
    if (repricedFee === feeSompi) {
      return Object.freeze({ ...prepared, feeSompi, mass, assumedSignedInputs, priorityFeerate, relayFloorRate: DEFAULT_RELAY_FLOOR_RATE });
    }
